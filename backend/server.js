...
// Import the getSecret functino from the secret.js file
...
// Use Mongoose to set up a connectinon to the database
...
app.use(bodyParser.urlencoded({ extended: false })); // Use for parsing data formatted in the x-www-form-urlencoded format
app.use(bodyParser.json()); // Use for parsing data formatted in the JSON format
app.use(logger('dev'));
...
router.post('/postData', (req, res) => {
    ...
    // If ID does not have a value and is not equal to 0 of message doesn't have a value, return an error
    if ((!id && id !== 0) || !message){
        return res.json({
            success: false,
            error: 'INVALID INPUT'
